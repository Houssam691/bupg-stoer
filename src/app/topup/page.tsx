...
export default async function TopupPage() {
  const products = (await readProducts()).filter((p) => p.category === "topup");

  return (
    <div className="grid gap-6">
      <section className="glass rounded-3xl p-6 md:p-10">
        <h1 className="title">خدمات الشحن (Top-up)</h1>
        <p className="subtitle">شحن شدات / جواهر / عملات بسرعة. اختر الخدمة ثم تواصل معنا للتأكيد.</p>
      </section>
          ...
          <article key={p.id} className="glass glass-hover overflow-hidden rounded-3xl">
            ...
            <div className="p-5">
              ...
              <div className="mt-4 flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between">
                ...
                <a className="btn-primary w-full sm:w-auto" href={`/chat/new?productId=${encodeURIComponent(p.id)}`}>اطلب الآن</a>
              </div>
            </div>
          </article>
        ))}
